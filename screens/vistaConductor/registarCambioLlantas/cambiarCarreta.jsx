...
import { cambioLlantasURL, obsURL } from "../../../api/apiurls";
...
export function CambiarCarreta() {
  ...
  const handleCambiar = (rowIndex, buttonIndex) => {
    const newCambios = cambios.map((row, rIndex) => row.map((button, bIndex) => rIndex === rowIndex && bIndex === buttonIndex));
  ...
  return (
    <>
      ...
      {cambios.some((row) => row.includes(true)) && (
        <>
          <Text style={[generalStyles.tittleText, { margin: 10 }]}>Registro de cambio {llantaSelect}</Text>
          <TextInput
          ...
          />
          <Button title={"Enviar"} onPress={() => handleEnviar()} buttonStyle={[generalStyles.styleButton, { marginVertical: 15 }]} />
        </>
      )}
              ...
              {Array.isArray(number) ? (
                  ...
                  {number.map((nestedNumber, nestedIndex) => (
                    <React.Fragment key={nestedIndex}>
                      <Button
                        ...
                        buttonStyle={[localStyles.button, cambios[rowIndex][buttonIndex] && localStyles.buttonSelected]}
                        onPress={() => handleCambiar(rowIndex, buttonIndex)}
                      />
                      {nestedIndex < number.length - 1 && <View style={localStyles.buttonSeparator} />}
                    </React.Fragment>
                  ))}
                </>
              ) : (
                <>
                  <Button
                    ...
                    onPress={() => handleCambiar(rowIndex, buttonIndex)}
                  />
                  {buttonIndex < row.length - 1 && <View style={localStyles.buttonSeparator} />}
                </>
              )}
            </React.Fragment>
          ))}
